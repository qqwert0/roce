...
class TX_EXH_FSM() extends Module{
	val io = IO(new Bundle{
        ...
        val msn2tx_rsp          = Flipped(Decoupled(new MSN_STATE()))
	...
	switch(state){
		is(sIDLE){
			when(event_fifo.io.deq.fire()){
				event_meta	                    <> event_fifo.io.deq.bits
            ...
            when(msn_tx_fifo.io.deq.fire()){
                ...
                switch(event_meta.op_code){
                    is(IB_OP_CODE.RC_WRITE_FIRST){
                        reth_head.length                := event_meta.length
                        reth_head.vaddr                 := event_meta.vaddr
                    ...
                    is(IB_OP_CODE.RC_READ_RESP_ONLY){
                        aeth_head.msn                   := msn_tx_fifo.io.deq.bits.msn
                        aeth_head.isNAK                 := 0.U

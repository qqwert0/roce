	...
	switch(state){
		is(sIDLE){
			when(ibh_data_fifo.io.deq.fire() & ibh_meta_fifo.io.deq.fire()){
                ...
                io.ibh_meta_out.bits.ibh_gene(IB_OP_CODE.safe(ibh_header_tmp.op_code)._1, ibh_header_tmp.qpn, ibh_header_tmp.psn, ibh_header_tmp.ack.asBool, ibh_meta_fifo.io.deq.bits.udp_length) 

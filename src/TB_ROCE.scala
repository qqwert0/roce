...
class TB_ROCE() extends Module{
	val io = IO(new Bundle{
        ...
        val status_reg          = Output(Vec(32,UInt(32.W)))
	})
    ...
    io.status_reg           := 0.U.asTypeOf(io.status_reg)


    io.status_reg(1)          := XCounter.record_signals_sync(io.m_mem_read_cmd.fire())
    io.status_reg(2)          := XCounter.record_signals_sync(io.s_mem_read_data.fire())
    io.status_reg(3)          := XCounter.record_signals_sync(io.m_mem_write_cmd.fire())
    io.status_reg(4)          := XCounter.record_signals_sync(io.m_mem_write_data.fire())
    io.status_reg(5)          := XCounter.record_signals_sync(io.m_net_tx_data.fire())
    io.status_reg(6)          := XCounter.record_signals_sync(io.s_net_rx_data.fire())
    io.status_reg(7)          := rd_time_cnt
    io.status_reg(8)          := wr_time_cnt

    for(i<-0 until 16){
        io.status_reg(i+16)             := roce.io.status_reg(i)
    }

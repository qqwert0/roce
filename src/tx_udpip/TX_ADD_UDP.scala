...
class TX_ADD_UDP() extends Module{
	...
	ReporterROCE.report(state===sIDLE, "TX_ADD_UDP===sIDLE")
	...
	switch(state){
		is(sIDLE){
			when(udp_meta_fifo.io.deq.fire() & udp_data_fifo.io.deq.fire()){
                udp_head.src_prot               := CONFIG.RDMA_DEFAULT_PORT.U
                udp_head.des_prot               := udp_meta_fifo.io.deq.bits.dest_port
                udp_head.length                 := udp_meta_fifo.io.deq.bits.udp_length
